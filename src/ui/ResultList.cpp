// ResultList — 搜索结果列表（状态文字在底部，高亮位置修复）
...
void ResultList::setResults(const std::vector<SearchResultEntry>& results)
{
    ...
    // 焦点位置尽量不变，仅当超出新结果范围时才回退
    if (m_results.empty()) {
        m_selectedIndex = -1;
    } else if (oldIdx < 0) {
        m_selectedIndex = 0;
    } else if (oldIdx >= (int)m_results.size()) {
        m_selectedIndex = 0;  // 旧焦点已超出新结果范围 → 回第一项
    } else {
        m_selectedIndex = oldIdx;  // 保持焦点不变！
    }

    m_scrollOffset = 0;
    m_scrollY = 0.0f; m_scrollTargetY = 0.0f;
    m_highlightAlpha = 1.0f;
    m_highlightTarget = 1.0f;

    // 同步高亮条位置（修复：需加上 m_pos.y，否则与绘制位置错位）
    if (m_selectedIndex >= 0) {
        float targetY = m_pos.y + (float)m_selectedIndex * m_itemHeight;
        m_highlightY = targetY;
        m_highlightTargetY = targetY;
    }
}
...
void ResultList::setSelectedIndex(int index)
{
    ...
    if (m_selectedIndex != oldIndex) {
        m_highlightAlpha = 0.4f; m_highlightTarget = 1.0f;
    }
    ...
    m_highlightTargetY = m_pos.y +
        (float)(m_selectedIndex - m_scrollOffset) * m_itemHeight;
...
bool ResultList::onMouseEvent(const MouseEvent& evt) {
    ...
    if (evt.type == MouseEvent::Press && evt.button == 0) {
        ...
        if (idx >= 0 && idx < (int)m_results.size()) { setSelectedIndex(idx); if (m_onConfirm) m_onConfirm(idx); return true; }
    }
    return false;
}
    ...
    if (m_results.empty()) {
        ...
        if (!m_statusText.empty() && m_fontMgr) {
            ...
            if (run.atlasHandle != kInvalidHandle && !run.vertices.empty()) {
                ...
                RenderCommand c; TextRunOp t;
                t.textureHandle = run.atlasHandle;
                t.vertices.resize(run.vertices.size());
                for (size_t v = 0; v < run.vertices.size(); ++v) {
                    t.vertices[v].pos = glm::vec2(run.vertices[v].pos.x + m_pos.x + m_paddingX, run.vertices[v].pos.y + oy);
                    t.vertices[v].uv = run.vertices[v].uv;
                }
    ...
    for (int i = startIdx; i < endIdx; ++i) {
        ...
        if (sel) {
            ...
            { RenderCommand c; RectOp r; r.pos = glm::vec2(m_pos.x+2, hlY+1); r.size = glm::vec2(m_size.x-4, m_itemHeight-2);
              r.color = glm::vec4(0.25f,0.45f,0.85f,0.4f * m_highlightAlpha); c.op = r; cmdBuf.add(c); }
            { RenderCommand c; RectOp r; r.pos = glm::vec2(m_pos.x+2, hlY); r.size = glm::vec2(3, m_itemHeight);
              r.color = glm::vec4(0.3f,0.6f,1.0f,m_highlightAlpha); c.op = r; cmdBuf.add(c); }
        }
        ...
        if (m_iconCache) {
            ...
            if (h != kInvalidHandle) { RenderCommand c; IconOp ic; ic.textureHandle = h; ic.pos = glm::vec2(iconX, iconY); ic.size = glm::vec2(m_iconSize); ic.tintColor = glm::vec4(1); c.op = ic; cmdBuf.add(c); }
        }
        float textX = iconX + m_iconSize + 8.0f;

        if (m_fontMgr && !result.name.empty()) {
            float maxW = m_size.x - textX - m_paddingX - 80;
            auto run = m_fontMgr->createTextRun(result.name, m_fontSize, maxW, glm::vec4(1));
            if (run.atlasHandle != kInvalidHandle && !run.vertices.empty()) {
                float oy = itemY + m_fontSize + 2;
                RenderCommand c; TextRunOp t; t.textureHandle = run.atlasHandle;
                t.vertices.resize(run.vertices.size());
                for (size_t v = 0; v < run.vertices.size(); ++v) {
                    t.vertices[v].pos = glm::vec2(run.vertices[v].pos.x + textX, run.vertices[v].pos.y + oy - m_fontSize);
                    t.vertices[v].uv = run.vertices[v].uv;
                }
                t.indices = std::move(run.indices); t.color = sel ? glm::vec4(1,1,1,1) : glm::vec4(0.8f,0.8f,0.8f,1); t.opacity = 1;
                c.op = t; cmdBuf.add(c);
            }
        }
        ...
        // 路径
        {
            ...
            auto run = m_fontMgr->createTextRun(info, pfs, maxW, glm::vec4(0.55f,0.55f,0.55f,1));
            if (run.atlasHandle != kInvalidHandle && !run.vertices.empty()) {
                float oy = itemY + pfs + 24;
                RenderCommand c; TextRunOp t; t.textureHandle = run.atlasHandle;
                t.vertices.resize(run.vertices.size());
                for (size_t v = 0; v < run.vertices.size(); ++v) {
                    t.vertices[v].pos = glm::vec2(run.vertices[v].pos.x + textX, run.vertices[v].pos.y + oy - pfs);
                    t.vertices[v].uv = run.vertices[v].uv;
                }
                t.indices = std::move(run.indices); t.color = glm::vec4(0.55f,0.55f,0.55f,1); t.opacity = 1;
                c.op = t; cmdBuf.add(c);
            }
        }
    }
